...
Route::get('/cetak-laporan-teknologi', function () {
    ...
    foreach ($composerPackages as $name => $version) {
        if (isset($libraryMap[$name])) {
            $laporan['backend'][] = ['name' => $name, 'version' => $version, 'desc' => $libraryMap[$name]];
        }
    }

    foreach ($npmPackages as $name => $version) {
        if (isset($libraryMap[$name])) {
            $laporan['frontend'][] = ['name' => $name, 'version' => $version, 'desc' => $libraryMap[$name]];
        }
    }
    ...
    <body>
        ...
        <table><thead><tr><th>Nama Package</th><th>Versi</th><th>Fungsi Utama</th></tr></thead><tbody>';

    foreach ($laporan['backend'] as $item) {
        $html .= '<tr><td><strong>' . $item['name'] . '</strong></td><td><span class="badge">' . $item['version'] . '</span></td><td>' . $item['desc'] . '</td></tr>';
    }

    $html .= '</tbody></table><h3>B. Frontend & UI (Node.js)</h3><table><thead><tr><th>Nama Package</th><th>Versi</th><th>Fungsi Utama</th></tr></thead><tbody>';

    foreach ($laporan['frontend'] as $item) {
        $html .= '<tr><td><strong>' . $item['name'] . '</strong></td><td><span class="badge">' . $item['version'] . '</span></td><td>' . $item['desc'] . '</td></tr>';
    }

    $html .= '</tbody></table><br><br><p style="text-align: right; font-size: 0.9em; color: #777;"><em>Generated automatically by System on ' . date('d F Y') . '</em></p></body></html>';
...
Route::prefix('api/db')->group(function () {
    Route::get('kabupaten', function () {
        ...
        return response()->json(['success' => true, 'data' => $kabupatens]);
    })->name('api.db.kabupaten');

    Route::get('kecamatan/{kabupaten_id}', function ($kabupaten_id) {
        $kecamatans = Kecamatan::where('kabupaten_id', $kabupaten_id)->orderBy('nama_kecamatan')->get();
    ...
    Route::get('posyandu/by-kecamatan/{kecamatan_id}', function ($kecamatan_id) {
        $posyandus = Posyandu::where('kecamatan_id', $kecamatan_id)->orderBy('nama_posyandu')->get(['id', 'nama_posyandu', 'desa']);
        return response()->json(['success' => true, 'data' => $posyandus]);
    })->name('api.db.posyandu.by-kecamatan');

    Route::get('posyandu/by-kabupaten/{kabupaten_id}', function ($kabupaten_id) {
        $posyandus = Posyandu::with('kecamatanRelation')->where('kabupaten_id', $kabupaten_id)->orderBy('nama_posyandu')->get(['id', 'nama_posyandu', 'desa', 'kecamatan_id']);
        return response()->json(['success' => true, 'data' => $posyandus]);
...
Route::get('/api/posyandu/{posyandu}/rw-rt', function (Posyandu $posyandu) {
    if (!$posyandu->rw_list || count($posyandu->rw_list) === 0) {
        return response()->json([
            ...
            'data' => ['rw_list' => [], 'rt_mapping' => [], 'total_rw' => 0, 'total_rt' => 0]
        ]);
    }
    return response()->json([
        'success' => true,
...
Route::get('/api/wilayah/posyandu', function (Request $request) {
    ...
    $posyandus = Posyandu::where('kabupaten', $request->query('kabupaten'))
        ->where('kecamatan', $request->query('kecamatan'))
        ->where('desa', $request->query('desa'))
        ->when($request->query('search', ''), fn($q, $s) => $q->where('nama_posyandu', 'like', "%{$s}%"))
...
Route::get('/track-submission', [AjuanController::class, 'showTrackingForm'])->name('ajuan.track.form');
Route::post('/track-submission', [AjuanController::class, 'track'])->name('ajuan.track');
Route::get('/lacak-pengajuan', [AjuanController::class, 'trackShow'])->name('ajuan.track.show');

Route::middleware(['auth'])->group(function () {
    Route::get('/ajuan/{ajuan}/print-bukti', [AjuanController::class, 'printBukti'])->name('ajuan.print-bukti');
});

Route::get('/api/guidebook', [BukuSakuController::class, 'getGuidebookForLogin'])->name('api.guidebook');
Route::get('/api/guidebook/{bukuSaku}/file', [BukuSakuController::class, 'streamGuidebookFile'])->name('api.guidebook.stream-file');
...
Route::middleware('auth')->group(function () {
    ...
    // Hanya menampilkan halaman — tidak ada data sensitif
    Route::get('/dashboard', [DashboardController::class, 'index'])->name('dashboard');

    // Endpoint AJAX — dipanggil JS, bukan dibuka langsung di browser
    ...
    Route::get('/pilih-layanan', [AjuanController::class, 'pilihLayanan'])->name('dashboard.partials.pilih-layanan');

    Route::get('/admin/ajuan/pilih-user', [AjuanController::class, 'pilihUser'])
        ->middleware('role:admin,kabid,kader,admin-kecamatan')
    ...
    Route::middleware(['auth', 'role:admin,admin-kabupaten'])->group(function () {
        ...
        Route::post('/admin/settings/reset', [SystemSettingController::class, 'reset'])->name('admin.settings.reset');
        Route::get('/api/settings/current', [SystemSettingController::class, 'getCurrent'])->name('api.settings.current');
    });
    ...
    Route::middleware(['auth'])->group(function () {
        Route::get('/ajuan/{ajuan}/print', [AjuanController::class, 'printBukti'])->name('ajuan.print');
        Route::get('/ajuan/{ajuan}/qrcode', [AjuanController::class, 'generateQRCode'])->name('ajuan.qrcode');
    ...
    Route::middleware('role:ketua-timpembina-posyandu,kabid,admin-kecamatan,kades,bu-kades,ketua-posyandu,operator-desa,kader,masyarakat')->group(function () {
        ...
        Route::get('/ajuan/{ajuan}/foto-kunjungan/{index}', [AjuanController::class, 'showFotoKunjungan'])->name('ajuan.foto-kunjungan');
        ...
        Route::post('/ajuan/{ajuan}/request-revision', [AjuanController::class, 'requestRevision'])->name('ajuan.request-revision');
    });
    ...
    Route::middleware(['role:kader,admin,admin-kabupaten,operator-desa'])->prefix('admin')->name('admin.')->group(function () {
        Route::resource('users', UserController::class);
        Route::patch('/users/{user}/verify', [UserController::class, 'verify'])->name('users.verify');

        Route::get('/users/import', fn() => view('admin.users.import'))->name('import.importPage');
        Route::post('/users/import', [UserController::class, 'importProcess'])->name('import.importProcess');
        Route::get('export-user-template', [UserController::class, 'exportTemplate'])->name('users.export.template');
    ...
    Route::middleware(['role:kabid,admin-kecamatan,ketua-posyandu,operator-desa,admin-kabupaten,admin,ketua-timpembina-posyandu'])->prefix('admin')->name('admin.')->group(function () {
        Route::middleware('role:admin,admin-kabupaten,ketua-timpembina-posyandu,admin-kecamatan')->group(function () {
            ...
            Route::get('export-posyandu-all', [PosyanduController::class, 'exportAllPosyandu'])->name('posyandu.export.all');
            Route::get('export-posyandu-kecamatan/{kecamatan}', [PosyanduController::class, 'exportByKecamatan'])->name('posyandu.export.kecamatan');
            Route::get('export-posyandu-desa/{desa}', [PosyanduController::class, 'exportByDesa'])->name('posyandu.export.desa');
            Route::post('export-posyandu', [PosyanduController::class, 'exportWithFilter'])->name('posyandu.export.filter');
    ...
    Route::middleware(['role:operator-desa'])->group(function () {
        Route::patch('admin/users/{user}/reset-password', [UserController::class, 'resetPasswordKader'])->name('admin.users.reset-password');
        Route::patch('admin/users/{user}/deactivate', [UserController::class, 'deactivateKader'])->name('admin.users.deactivate');
        Route::patch('admin/users/{user}/reactivate', [UserController::class, 'reactivateKader'])->name('admin.users.reactivate');
    });

    // ========================================
    // ADMIN ROUTES - KETUA POSYANDU (TAKEOVER KADER)
    // ========================================

    Route::middleware(['auth', 'role:ketua-posyandu'])->prefix('ketua-posyandu')->name('ketua-posyandu.')->group(function () {
    ...
    Route::middleware('role:operator-desa,admin,ketua-posyandu,admin-kabupaten')->group(function () {
        Route::get('/posyandu/{posyandu}/edit-rw-rt', [PosyanduController::class, 'editRwRt'])->name('admin.posyandu.edit-rw-rt');
        Route::put('/posyandu/{posyandu}/update-rw-rt', [PosyanduController::class, 'updateRwRt'])->name('admin.posyandu.update-rw-rt');
        Route::get('/posyandu/{posyandu}/manage-rw-rt', [PosyanduController::class, 'manageRwRt'])->name('admin.posyandu.manage-rw-rt');
        Route::post('/posyandu/{posyandu}/save-rw-rt', [PosyanduController::class, 'saveRwRt'])->name('admin.posyandu.save-rw-rt');
    });
});
